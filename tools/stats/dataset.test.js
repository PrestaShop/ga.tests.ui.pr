...
test('an unknown dataset version is refused rather than misread', () => {
  assert.throws(() => decodeDataset({ v: DATASET_VERSION + 1 }), /Unsupported dataset version/);
  assert.throws(() => decodeDataset(null), /Unsupported dataset version/);
});
...
test('the CSV export has one row per execution and quotes what it must', () => {
  ...
  const tricky = toCsv([
    { owner: 'o', run_id: 1, html_url: 'u', workflow: 'w', branch_key: 'b', branch_key_source: 's', created_at: 'c', aborted: false, executions: [{ campaign: 'a,b "quoted"', attempt: 1, conclusion: 'success' }] },
  ]);
