...
import type { MaybeRetryable, Transport } from './github.js';
...
export function cachingTransport({ dir, mode, upstream = globalThis.fetch }: CacheOptions): Transport {
  return {
    async fetch(url, init) {
      ...
      if (mode === 'replay') {
        ...
        if (stored === null) {
          const missing: Error & MaybeRetryable = new Error(
            `No recorded response for ${String(url)} (looked in ${file})`,
          );
        ...
        return toResponse(JSON.parse(stored) as RecordedResponse);
      }
      ...
      await writeFile(
        file,
        JSON.stringify({ url: String(url), status: res.status, headers, body }),
        'utf8',
      );
      return toResponse({ status: res.status, headers, body });
    },
  };
}

/** The range header is part of the identity: a head slice is not the same response as a whole log. */
function keyFor(url: Parameters<typeof globalThis.fetch>[0], init?: RequestInit): string {
  const headers = (init?.headers ?? {}) as Record<string, string>;
  const range = headers.range ?? headers.Range ?? '';
  return createHash('sha256').update(`${String(url)}\n${range}`).digest('hex').slice(0, 32);
}

function toResponse({ status, headers, body }: RecordedResponse): Response {
