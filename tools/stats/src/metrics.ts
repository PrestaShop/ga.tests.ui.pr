/**
 * Flaky-test statistics. Pure functions, no imports beyond types: this module is loaded both
 * by the aggregator in Node and by the dashboard in the browser, so the numbers on the page
 * and the numbers in the generated summary can never drift apart.
 *
...
export function campaignOutcomes(run: Pick<DecodedRun, 'executions'>): CampaignOutcome[] {
  const byCampaign = new Map<string, Execution[]>();
  for (const exec of run.executions ?? []) {
    const bucket = byCampaign.get(exec.campaign);
    if (bucket) bucket.push(exec);
    else byCampaign.set(exec.campaign, [exec]);
  }

  const out: CampaignOutcome[] = [];
  for (const [campaign, execs] of byCampaign) {
    const ordered = [...execs].sort((a, b) => a.attempt - b.attempt);
    const rated = ordered.filter((e) => e.conclusion !== null && OUTCOMES.has(e.conclusion));
    const first = rated[0];
    ...
    out.push({
      ...
      firstFailed,
      attempts: ordered.length ? (ordered[ordered.length - 1]?.attempt ?? 0) : 0,
      final,
      flaky,
      hardFailure,
 ...
 *
 * @returns seconds, 0 when nothing has usable timestamps
 */
export function wallClockSeconds(executions: Execution[]): number {
  let first = Infinity;
  let last = -Infinity;
 ...
 */
export function runTiming(executions: Execution[]): RunTiming {
  const byAttempt = new Map<number, Execution[]>();
  for (const exec of executions ?? []) {
    if (!exec.started_at) continue;
    const bucket = byAttempt.get(exec.attempt);
    if (bucket) bucket.push(exec);
    else byAttempt.set(exec.attempt, [exec]);
  }
  ...
  return {
    ...
    firstAttemptSeconds: wallClockSeconds(firstAttempt[1]),
    attemptCount: attempts.length,
  };
}

/** How healthy a whole test run is, independent of which campaign misbehaved. */
export function runLevelStats(runs: DecodedRun[]): RunLevelStats {
 ...
 */
export function campaignStats(runs: DecodedRun[]): CampaignRow[] {
  const acc = new Map<string, CampaignAccumulator>();

  /** Durations of successful executions only: `--bail` cuts a failing campaign short. */
  const successDurations = new Map<string, number[]>();

  for (const run of runs) {
    if (run.aborted) continue;
    for (const exec of run.executions ?? []) {
      if (exec.conclusion !== 'success' || !exec.duration_s) continue;
      const bucket = successDurations.get(exec.campaign);
      if (bucket) bucket.push(exec.duration_s);
      else successDurations.set(exec.campaign, [exec.duration_s]);
    }
...
export function weeklyTrend(runs: DecodedRun[]): WeekRow[] {
  const weeks = new Map<string, DecodedRun[]>();
  for (const run of runs) {
    const week = isoWeekStart(run.created_at);
    if (!week) continue;
    const bucket = weeks.get(week);
    if (bucket) bucket.push(run);
    else weeks.set(week, [run]);
  }
...
export function filterRuns(runs: DecodedRun[], filters: Filters = {}): DecodedRun[] {
...
export function isoWeekStart(timestamp: string | null | undefined): string | null {
...
/**
 * The same division, but only between two counts of the same thing.
 *
 * This exists because of the review: `pct(row.failures, campaignFailures)` divided scenario
 * namings by campaign executions, two different units that are both numbers, and three
 * campaigns reported more than 100% of their own failures. Branding the counts means writing
 * that expression again does not compile.
 */
function shareOfExecutions(part: ExecutionCount, total: ExecutionCount): number {
...
interface ScenarioAccumulator {
  ...
  /** Times this scenario was named. */
  failures: ScenarioNamings;
  /** Execution-equivalents: `1/n` per scenario of an execution that named n of them. */
  executionShare: ExecutionCount;
 ...
 *
 * The share is a share of that campaign's failing executions, which is the number that says
 * where to start: a campaign failing 47% of the time because of one scenario is a different
 * job from one failing for a dozen reasons.
 *
 * ⚠️ Two different units meet here, which is why they are branded. `--bail` is usually on, so
 * a red execution names exactly one scenario and the two coincide — but it is not always on,
 * and an execution that names three scenarios produces three rows. Counting each of them as
 * a whole failure against a denominator of failing *executions* is what once made three
 * campaigns report 175%, 150% and 105% of their own failures. So `failures` stays an honest
 * count of the times a scenario was named, while the share divides `executionShare` — the
 * execution credited as `1/n` across the n scenarios it blamed — by the execution total.
 * Those two are in the same unit, so the shares sum to at most 100, and what is left over is
 * exactly the infra and expired-log remainder the caveat line reports.
 *
 ...
 *
 * @param campaign restrict to one campaign; omit for every campaign at once
 */
export function scenarioStats(runs: DecodedRun[], campaign?: string): ScenarioStats {
  const acc = new Map<string, ScenarioAccumulator>();
  let campaignFailures = executionCount(0);
  let attributed = 0;
  let infraFailures = 0;
  ...
  for (const run of runs) {
    ...
    for (const exec of run.executions ?? []) {
      ...
      campaignFailures = executionCount(campaignFailures + 1);
      ...
      for (const test of failing) {
        ...
        if (!row) {
          row = {
            ...
            line: test.line ?? null,
            campaigns: new Set(),
            failures: scenarioNamings(0),
            executionShare: executionCount(0),
            flakyFailures: 0,
            prs: new Set(),
            errors: new Map(),
            lastFailureAt: null,
        ...
        row.failures = scenarioNamings(row.failures + 1);
        row.executionShare = executionCount(row.executionShare + 1 / failing.length);
  ...
  const scenarios: ScenarioRow[] = [...acc.values()]
    .map((row) => ({
      title: row.title,
      suite: row.suite,
      file: row.file,
      ...
      shareOfFailuresPct: shareOfExecutions(row.executionShare, campaignFailures),
