...
import { collect, isTestWorkflow, isFinal, TEST_WORKFLOWS } from './collect.js';
import { RateLimitReached } from './github.js';
 ...
 */
class FakeGitHub {
  constructor({ forks = [], runs = {}, jobs = {}, failRepos = {}, failRuns = {} } = {}) {
    this.forks = forks;
    this.runs = runs; //       repo -> run rows, or null for "unreadable"
    this.jobs = jobs; //       run id -> job rows
    this.failRepos = failRepos; // repo -> Error to throw when listing
    this.failRuns = failRuns; //   run id -> Error to throw when reading jobs
    this.requestCount = 0;
    this.seenRuns = [];
    this.onListJobs = null;
  }
  ...
  async listDispatchRuns(repo) {
    this.requestCount += 1;
    if (this.failRepos[repo]) throw this.failRepos[repo];
  ...
  async listRunJobs(repo, runId) {
    ...
    if (this.failRuns[runId]) throw this.failRuns[runId];
...
const jobRows = (id, name = 'test (audit, develop)') => [
  {
    id: id * 10,
    run_attempt: 1,
    name,
...
const collectFrom = (github, store, extra = {}) =>
  collect({ github, store, repos: [REPO], withLogs: false, withTests: false, ...extra });
...
test('only what is new or has gained an attempt is queued', async () => {
  await withStore(async (dir, store) => {
    ...
    await store.saveRun(REPO, { run_id: 1, run_attempt: 1, status: 'completed', executions: [] });
    await store.saveRun(REPO, { run_id: 2, run_attempt: 1, status: 'completed', executions: [] });
